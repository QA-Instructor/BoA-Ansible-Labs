...
def create_sandbox(body: SandBoxCreate, response: Response, authorization: Annotated[HTTPAuthorizationCredentials, Depends(auth.security)]):
...
def get_sandbox(uuid: UUID, response: Response, authorization: Annotated[HTTPAuthorizationCredentials, Depends(auth.security)]):
...
def patch_sandbox(if_match: Annotated[str | None, Header()], uuid: UUID, body: SandBoxCreate, response: Response, authorization: Annotated[HTTPAuthorizationCredentials, Depends(auth.security)]):
...
def delete_sandbox(uuid: UUID, response: Response, authorization: Annotated[HTTPAuthorizationCredentials, Depends(auth.security)]):
    if not uuid:
        response.status_code = status.HTTP_400_BAD_REQUEST
        return {"detail": "no sandbox ID specified"}
    for sb in store["sandboxes"]:
